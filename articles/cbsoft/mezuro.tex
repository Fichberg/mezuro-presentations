...
\begin{abstract}
  ...
  \\Adicionar link para o vídeo no youtube
\end{abstract}
...
\end{itemize}
  ...
  \begin{itemize}
    ...
    \item Modularizar a aplicação em diversos serviçoes independentes
  \end{itemize}
  ...
  As elipses são os diferentes softwares envolvidos e os paralelogramos as interfaces de comunicação entre estes. Na base do Mezuro existe o Kalibro, que está sendo reescrito 
  ...
  O objetivo pretendido com quebra da estrutura monolitica do Kalibro é que sua manutenção e evolução torne-se mais fácil, sem que todo o sistema venha a ser comprometido com tal ação.
...
\section{Conclusão}
Já existem diversas ferramentas que extraem informação sobre a qualidade do código-fonte, porém nenhuma delas realiza as funções que gostaríamos. Como existem divergências na classificação do que é um código bom ou um código ruim, propomos uma livre discussão e aprimoramento de configurações de métricas, buscando um consenso entre desenvolvedores para encontrar por um método evolutivo melhores maneiras para monitorar o código-fonte de uma maneira automatizada, confiável e significativa. 
